 ...
 */
public class PersonnelDAO extends UserDAO {
	...
	public String getName(long mid) throws ITrustException, DBException {
		return getName(mid, factory, "personnel");
	}
	...
	// New method added by Carolyn Thompson - 2/1/2017 to get only HCP list
	// specifically, like above method but tweaked
	// This is for creating an HCP drop down list in appointmentRequests.jsp so
	// the patient can choose the HCP for appointment
	/**
	 * Returns all HCPs in the database.
	 * 
	 * @return A java.util.List of HPSs.
	 * @throws DBException
	 */
	public List<PersonnelBean> getAllHCP() throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement stmt = conn.prepareStatement("SELECT * FROM personnel where role in ('hcp') ")) { // changed
																													// this
																													// to
																													// only
																													// 'hcp'
			final ResultSet results = stmt.executeQuery();
			final List<PersonnelBean> allHCP = personnelLoader.loadList(results); // changed
																					// var
																					// name
			results.close();
			return allHCP; // changed var name
		} catch (SQLException e) {
			throw new DBException(e);
		}
	}
