...
public class PatientDAO extends UserDAO {
	...
	public String getName(long mid) throws ITrustException, DBException {
		return getName(mid, factory, "patients");
	}
	...
	public void removeAllRepresented(long representerMID) throws DBException {
		try (Connection conn = factory.getConnection();
				...
				PreparedStatement deleteStatement = conn
						.prepareStatement("DELETE FROM representatives WHERE representerMID=?")) {
	...
	public void removeAllRepresentee(long representeeMID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement deleteStatement = conn
						.prepareStatement("DELETE FROM representatives WHERE representeeMID=?");
				PreparedStatement updateStatement = conn
						.prepareStatement("UPDATE users SET isDependent=0 WHERE MID=?")) {
	...
	public List<PatientBean> getRenewalNeedsPatients(long hcpMID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn.prepareStatement("SELECT * FROM ( "
						...
						+ " WHERE" + " declaredhcp.HCPID = ?" + " AND patients.MID = declaredhcp.PatientID"
						...
						+ " OR (ovdiagnosis.ICDCode >= ? AND ovdiagnosis.ICDCode < ?)))" + " UNION ALL"
						+ " SELECT DISTINCT patients.* From patients, declaredhcp, ovdiagnosis, officevisits, ovmedication "
						+ " WHERE" + " declaredhcp.HCPID = ?" + " AND patients.MID = declaredhcp.PatientID"
						...
						+ " AS final" + " GROUP BY final.MID HAVING COUNT(*) = 2"
	...
	public int addDesignatedNutritionist(long patientMID, long HCPID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("INSERT INTO " + "designatedNutritionist(PatientID, HCPID) VALUES(?,?);")) {
	...
	public long getDesignatedNutritionist(long patientMID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("SELECT HCPID FROM " + "designatedNutritionist WHERE PatientID = ?;")) {
			ps.setLong(1, patientMID);
	...
	public int updateDesignatedNutritionist(long patientMID, long HCPID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("UPDATE designatedNutritionist " + "SET HCPID = ? WHERE PatientID = ?;")) {
			ps.setLong(1, HCPID);
	...
	public int deleteDesignatedNutritionist(long patientMID) throws DBException {
		try (Connection conn = factory.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("DELETE FROM designatedNutritionist " + "WHERE PatientID = ?;")) {
			ps.setLong(1, patientMID);
