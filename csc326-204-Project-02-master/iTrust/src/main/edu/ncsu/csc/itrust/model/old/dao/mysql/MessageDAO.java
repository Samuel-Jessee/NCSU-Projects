...
public class MessageDAO {
	...
	public List<MessageBean> getMessagesNameOrder(long mid, boolean order) throws SQLException, DBException {
		String strOrder = "ASC";
		if (!order)
			strOrder = "DESC";
		try (Connection conn = factory.getConnection();
				PreparedStatement stmt = (mid >= 999999999)
						? conn.prepareStatement(
								"SELECT message.* FROM message, patients WHERE message.from_id=patients.mid AND message.to_id=? ORDER BY patients.lastName " + strOrder + ", patients.firstName " + strOrder + ", message.sent_date " + strOrder + "")
						: conn.prepareStatement(
								"SELECT message.* FROM message, personnel WHERE message.from_id=personnel.mid AND message.to_id=? ORDER BY personnel.lastName " + strOrder + ", personnel.firstName " + strOrder + ", message.sent_date " + strOrder + "")) {
			stmt.setLong(1, mid);
			ResultSet rs = stmt.executeQuery();
	...
	public List<MessageBean> getMessagesNameAscending(long mid) throws SQLException, DBException {
		return getMessagesNameOrder(mid, true);
	}
	...
	public List<MessageBean> getMessagesNameDescending(long mid) throws SQLException, DBException {
		return getMessagesNameOrder(mid, false);
	}
