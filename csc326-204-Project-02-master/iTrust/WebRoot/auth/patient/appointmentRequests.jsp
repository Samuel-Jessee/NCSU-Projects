...
<%
	...
	//Added list for hcp that will come from a method in apptTypeDAO based on getAllPersonnel - 2/1/2017
	PersonnelDAO personnelDAO = prodDAO.getPersonnelDAO();
	List<PersonnelBean> hcpList = personnelDAO.getAllHCP();
	
	String msg = "";
	long hcpid = 900000000L; //should this be this value?
	String comment = "";
	String date = "";
	String hourI = "";
	String minuteI = "";
	...
	if (request.getParameter("request") != null) {
		...
		hcpid = Long.parseLong(request.getParameter("lhcp")); //uncommmented this, Carolyn - 2/1/2017 for Bug 4
...
<form action="appointmentRequests.jsp" method="post">
	<p>HCP:</p> <%-- there is no code to actually get input for the HCP here - 2/1/2017 --%>
	<%--start of my code additions - 2/1/1017 --%>
	<%--I checked MySQL Database to make sure that the drop down list contained only HCP personnel --%>
	<select name="lhcp">
		<%
			for (PersonnelBean hcp : hcpList) {
		%><option
			<%--<%if (hcp.getAMID() == (hcpid)) //is AMID the same as hcpid? What is AMID? --%>
			out.print("selected='selected'");
			value="<%=hcp.getMID()%>"><%=hcp.getFullName()%></option>
		<%
			}
			//String startDate = "";
			//based pn Appointment Type code below
		%>
	</select>
