...
/**
 * Container for the BugTracker that has the menu options for new bug tracking
 * files, loading existing files, saving files and quitting. Depending on user
 * actions, other {@link JPanel}s are loaded for the different ways users
...
public class BugTrackerGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a {@link BugTrackerGUI} object that will contain a
	 * {@link JMenuBar} and a {@link JPanel} that will hold different possible
	 * views of the data in the {@link BugTrackerModel}.
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == itemNewBugXML) {
			...
			try {
				model.saveBugsToFile(getFileName());
				System.exit(0); // Ignore FindBugs warning here - this is the
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that shows the list of bugs.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class BugListPanel extends JPanel implements ActionListener {
		...
		/**
		 * {@link BugTableModel} is the object underlying the {@link JTable}
		 * object that displays the list of {@link TrackedBug}s to the user.
		 * 
		 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
		 */
		private class BugTableModel extends AbstractTableModel {
			...
			/**
			 * Constructs the {@link BugTableModel} by requesting the latest
			 * information from the {@link BugTrackedModel}.
			 */
			...
			/**
			 * Updates the given model with {@link TrackedBug} information from
			...
			/**
			 * Updates the given model with {@link TrackedBug} information for
			 * the given owner from the {@link BugTrackerModel}.
			 * 
			 * @param owner
			 *            developer id to search for.
			 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with an unconfirmed bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class UnconfirmedPanel extends JPanel implements ActionListener {
		...
		private static final long serialVersionUID = 1L;
		/**
		 * {@link BugInfoPanel} that presents the {@link TrackedBug}'s
		...
		/**
		 * Constructs the JPanel for editing a {@link TrackedBug} in the
		 * UnconfirmedState.
		 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with an new bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class NewPanel extends JPanel implements ActionListener {
		...
		private static final long serialVersionUID = 1L;
		/**
		 * {@link BugInfoPanel} that presents the {@TrackedBug}'s information to
		 * the user
		 */
		private BugInfoPanel pnlBugInfo;
		/** Note label for the state update */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAssign) {
				...
				if (developerId == null || developerId.equals("")) {
					...
					// Otherwise, try a Command. If command fails, go back to
					// bug list
					try {
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with an assigned bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class AssignedPanel extends JPanel implements ActionListener {
		...
		private JComboBox<String> comboResolved;
		/** Resolve action */
		...
		/**
		 * Constructs a JPanel for editing a {@link TrackedBug} in the
		 * AssignedState.
		 */
		public AssignedPanel() {
			...
			comboResolved = new JComboBox<String>(resolutions);
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a resolved bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class ResolvedPanel extends JPanel implements ActionListener {
		...
		/**
		 * Constructs a JFrame for editing a {@link TrackedBug} in the
		 * ResolvedState.
		 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a reopened bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a closed bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that shows information about the bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class BugInfoPanel extends JPanel {
		 ...
		 * 
		 * @param bugId
		 *            the id for the bug to display information about.
		 */
		public void setBugInfo(int bugId) {
			// Get the bug from the model
			TrackedBug b = BugTrackerModel.getInstance().getBugById(bugId);
			if (b == null) {
				// If the bug doesn't exist for the given id, show an error
				// message
				JOptionPane.showMessageDialog(BugTrackerGUI.this, "Invalid bug id");
				cardLayout.show(panel, BUG_LIST_PANEL);
				BugTrackerGUI.this.repaint();
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that allows for creation of a new bug.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class CreateBugPanel extends JPanel implements ActionListener {
		...
		/**
		 * Creates the {@link JPanel} for adding new bugs to the tracker.
		 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAdd) {
				...
				try {
					...
					JOptionPane.showMessageDialog(BugTrackerGUI.this, "Invalid bug information.");
				}
			}
			if (done) {
				// All buttons lead to back bug list
