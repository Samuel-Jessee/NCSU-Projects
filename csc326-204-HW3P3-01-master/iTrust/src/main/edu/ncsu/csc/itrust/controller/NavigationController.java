...
public class NavigationController {
	...
	public void redirectIfInvalidPatient() throws DBException, IOException {
		...
		if (pidObj instanceof Long) {
			...
			pid = Long.valueOf((String) pidObj).longValue();
		}
	...
	public static void editFitnessHealth() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp/editFitnessHealth.xhtml");
		}
	}

	public static void editFitnessHealth(Long fitnessID) throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp/editFitnessHealth.xhtml?fitnessID=" + fitnessID);
		}
	}

	public static void viewFitnessHealth() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp/viewFitnessHealthInfo.xhtml");
		}
	}

	public static void baseFitnessHealth() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp/fitnessMain.xhtml");
		}
	}
}
