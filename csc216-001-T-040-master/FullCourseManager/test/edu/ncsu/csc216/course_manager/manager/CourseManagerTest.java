...
/**
 * Tests the CourseManager class. Because CourseManager is a Singleton, the
 * clearData() method must be called to ensure that each test starts with an
 * empty CourseManager and that each test runs independently.
 * 
 * @author SarahHeckman
 */
public class CourseManagerTest {
	 ...
	 * 
	 * @param sourcePath
	 *            path to source file - the correct copy
	 * @param destinationPath
	 *            path to destination file - the one to overwrite for future
	 *            tests
	 */
	...
	public void testAddCourse() {
		assertTrue(manager.addCourse(new Course("CSC116", 3, 7)));
		Course[] courses = manager.listAllCourses();
		...
		assertFalse(manager.addCourse(new Course("CSC116", 3, 3)));
		assertEquals(1, courses.length);
	...
	/**
	 * Tests CourseManager.removeCourse()
	 */
	@Test
	public void testRemoveCourse() {
		...
		assertTrue(manager.removeCourse(c2));
		Course[] courses = manager.listAllCourses();
		assertEquals(1, courses.length);
		...
		// Attempt to remove the same course again
		assertFalse(manager.removeCourse(c2));
		assertEquals(1, courses.length);
	...
	public void testLoadFaculty() {
		...
		// Check that correct faculty is assigned to CSC116
		Faculty f = c1.getFaculty();
		assertEquals(validFaculty[0], f.toString());

		// Check that correct faculty is assigned to CSC216
		f = c2.getFaculty();
		assertEquals(validFaculty[1], f.toString());

		// Check that correct faculty is assigned to CSC226
		f = c3.getFaculty();
		assertEquals(validFaculty[2], f.toString());

		// Check that correct faculty is assigned to CSC230
		f = c4.getFaculty();
		assertEquals(validFaculty[1], f.toString());

		// Check that correct faculty is assigned to CSC236
		f = c5.getFaculty();
		assertEquals(validFaculty[3], f.toString());

		// Check that correct faculty is assigned to CSC246
		f = c6.getFaculty();
		assertEquals(validFaculty[5], f.toString());

		// Check that correct faculty is assigned to CSC316
		f = c7.getFaculty();
		assertEquals(validFaculty[6], f.toString());

		// Check that correct faculty is assigned to CSC379
		f = c8.getFaculty();
		assertEquals(validFaculty[7], f.toString());
	}// testLoadFaculty

	/**
	 * Tests CourseManager.saveCourses(). Test is similar to
	 * CourseRecordsIOTest, but writes the courses stored in the CourseManager's
	 * list.
	 */
	@Test
	...
	/**
	 * Tests CourseManager.saveStudent(). Test is similar to
	 * StudentRecordsIOTest, but writes the students stored in the
	 * CourseManager's list.
	 */
	@Test
	public void testSaveStudents() {
		...
		// If you mess up the student_records.txt file, you can copy
		// expected_full_student_records.txt'ss
	...
	/**
	 * Tests CourseManager.saveFaculty(). Test is similar to
	 * FacultyRecordsIOTest, but writes the faculty stored in the
	 * CourseManager's list.
	 */
	@Test
	...
	public void testLogin() {
		...
		// attempts to log in as a Faculty
