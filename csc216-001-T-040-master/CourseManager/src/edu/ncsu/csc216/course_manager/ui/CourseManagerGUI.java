...
public class CourseManagerGUI extends JFrame {
	...
	private class InitPanel extends JPanel implements ActionListener {
		...
		private String getFileName() {
			JFileChooser fc = new JFileChooser("./"); // Open JFileChoose to
														// current working
														// directory
	...
	private class StudentPanel extends JPanel implements ActionListener {
		...
		public StudentPanel() {
			...
			listScrollerAllCourses = new JScrollPane(listAllCourses, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnAddCourse) {
				try {
					...
					if (c == null) {
						JOptionPane.showMessageDialog(this, "Select a course from the All Courses list.", "Error",
								JOptionPane.ERROR_MESSAGE);
					} else if (manager.addUserToCourse(c)) {
						JOptionPane.showMessageDialog(this, "Course successfully added.", "Success",
								JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(this, "Course cannot be added.", "Error",
								JOptionPane.ERROR_MESSAGE);
					}
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(this, iae.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
				...
				try {
					...
					if (c == null) {
						JOptionPane.showMessageDialog(this, "Select a course from the My Courses list.", "Error",
								JOptionPane.ERROR_MESSAGE);
					} else if (manager.removeUserFromCourse(listMyCourses.getSelectedValue())) {
						JOptionPane.showMessageDialog(this, "Course successfully removed.", "Success",
								JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(this, "Course cannot be added.", "Error",
								JOptionPane.ERROR_MESSAGE);
		...
		/**
		 * ListSelectionListener for MyCourses components. Will update the
		 * txtCourseDetails component when a course is selected from
		 * listMyCourses.
		 * 
		 * @author SarahHeckman
		 */
		private class MyCoursesListSelectionListener implements ListSelectionListener {
			...
			/**
			 * Method executes any time the selection in the list changes. The
			 * course details text area will update with the selected course
			 * info.
			 */
			@Override
		...
		/**
		 * ListSelectionListener for AllCourses components. Will update the
		 * txtCourseDetails component when a course is selected from
		 * listAllCourses.
		 * 
		 * @author SarahHeckman
		...
		/**
		 * Custom ListCellRenderer that only displays the course name and not
		 * the credits or max enrollment.
		 */
		private class NameCellRenderer extends JLabel implements ListCellRenderer<Course> {
			...
			/**
			 * Overrides the cell renderer for the lists so that only the
			 * course's name displays.
			 */
			@Override
			public Component getListCellRendererComponent(JList<? extends Course> list, Course value, int index,
